...
ANSWERS = {
    ...
    6 => "Réponse : Il y a #{JOURNALISTS.count{ |i| i[/_/]}} underscore _ dans tous le tableau de pseudos",
    7 => "Réponse : Voici la liste de handle trier par ordre alphabétique : #{JOURNALISTS.sort}",
    8 => "Réponse : Voici la liste des 50 handles les plus courts de ce tableau : #{JOURNALISTS.sort_by{|s| s.length }[0..50]}",
    9 => "Réponse : @epenser se trouve en #{JOURNALISTS.index('@epenser')} position"
}

def display_menu
  puts "- Bonjour & bon chance pour tes questions !"
  print ">"
  choice = gets.to_i
end

def ask_questionselsif choice === 1
